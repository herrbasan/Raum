...
6. **Analytics** — **DECIDED: yes, and live: nAlytics** (own service, extracted from nPort 2026-09-16; nPort is the thin edge in front, endpoint `POST https://nport.raum.com/analytics/ping`). Cookieless, no banner: one `fetch`, nothing read or written on the device, daily-salted visit dedup server-side, DNT/GPC visitors not measured at all. The beacon is baked into every page by `beacon()` in `tools/lib/pages.mjs` (as high in `<head>` as the charset allows, so short visits count) — the fields are `path`, `referrer`, `lang`, `w`, `h`, `dpr`, `conn`, everything else derived server-side; `path` is the only required one. Adding anything to the snippet that touches the visitor's device turns it into a consent-banner problem. Accepted local artefact: on the home LAN the request makes Chrome raise its Local Network Access prompt (nport.raum.com resolves privately there) — see the comment at the removal-and-restore site in `pages.mjs` before touching it. Spec: nAlytics `docs/beacon-snippet.md`; origins must be in nAlytics' `ORIGINS` allowlist.
...
- **Keep submodules up to date:** `modules/nui_wc2/` is a git submodule pinned to a snapshot of `herrbasan/nui_wc2`. It drifts — the vendored pin is often many commits behind `origin/main`. When working here, check `git submodule status` and compare against the submodule's remote; commit the submodule's own changes (e.g. a `nui-media-player` fix) inside `modules/nui_wc2/` first, push them to its repo, then bump the pointer in `Raum` (separate commit). Refreshing the whole vendored snapshot to `origin/main` is a bigger change (it can alter `tools/lib/md.mjs` markdown output + rendered behavior) — do that deliberately with a rebuild, not accidentally.
